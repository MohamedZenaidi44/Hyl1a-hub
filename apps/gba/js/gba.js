...
function renderGbaMenu(container) {
  if (!document.getElementById('gba-simple-cover-styles')) {
    ...
    style.textContent = `
      ...
      .gba-btn.primary {
        ...
        box-shadow: 0 0 20px rgba(124, 58, 237, 0.5); width: 240px; justify-content: center;
      }
  ...
  if (GBA_GAMES.length > 0) {
    ...
    const keyHandler = (e) => {
      ...
      if (!menuWrapper) { window.removeEventListener('keydown', keyHandler, true); return; }
      ...
      if (keys.includes(e.key)) {
        ...
        if (e.key === 'ArrowRight') {
          if (currentGbaCoverIndex < GBA_GAMES.length - 1) { currentGbaCoverIndex++; if (typeof AudioManager !== 'undefined') AudioManager.playClick(); updateGbaCarousel(container); }
        } else if (e.key === 'ArrowLeft') {
          if (currentGbaCoverIndex > 0) { currentGbaCoverIndex--; if (typeof AudioManager !== 'undefined') AudioManager.playClick(); updateGbaCarousel(container); }
        } else if (e.key === 'Enter') {
          if (typeof AudioManager !== 'undefined') AudioManager.playClick();
          launchGbaEmulator(container, GBA_GAMES[currentGbaCoverIndex]);
  ...
  container.querySelector('#gba-quit-btn').addEventListener('click', () => {
    if (typeof AudioManager !== 'undefined') AudioManager.playClick();
    if (window._gbaKeyHandler) { window.removeEventListener('keydown', window._gbaKeyHandler, true); window._gbaKeyHandler = null; }
...
function updateGbaCarousel(container) {
  ...
  items.forEach((item, index) => {
    item.classList.toggle('active', index === currentGbaCoverIndex);
  });
