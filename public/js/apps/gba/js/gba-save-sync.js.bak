/**
   ...
   *   - attend l’événement firebase-ready avant de démarrer
   */
  (function () {
    ...
    /* Clé IndexedDB où l’émulateur stocke la SRAM
       Vérifiée précédemment : "/data/saves/mGBA/Pok%C3%A9mon%20-%20Version%20Emeraude%20(France).srm" */
    const idbKey = `/data/saves/mGBA/${srmFileName}`;

    /* ----- Intervalles ----- */
    const POLL_INTERVAL_MS = 2000;   // fréquence de vérification des changements dans IDB
    const FORCED_UPLOAD_MS = 30_000; // upload de secours (au cas où le polling manquerait quelque chose)

    /* ----- État ----- */
    let lastSaveHash = null; // hash de la dernière sauvegarde que nous avons uploadée
    let cloudSaveHash = null; // hash de ce que nous avons téléchargé depuis le worker
        ...
        req.onsuccess = (e) => {
          ...
          get.onsuccess = () => {
            ...
            console.log(`[SaveSync] 📖 Lecture IDB terminée – clé=${idbKey}, résultat=`,
                        result ? `objet (timestamp=${result.timestamp}, mode=${result.mode},
  longueur=${result.contents.byteLength})` : 'null');
            resolve(result);
          };
          get.onerror   = () => {
    ...
    function waitForAuth(cb) {
      ...
      const check = setInterval(() => {
        tries++;
        const auth = window.FirebaseAuth; // on utilise l'export globale
    ...
    async function downloadAndInjectSave(user) {
      try {
        ...
        lastSaveHash  = cloudSaveHash; // on vient de sync, pas besoin de re‑upload maintenant
    ...
    async function uploadSaveIfChanged(user) {
      try {
        ...
        // Si aucune modification, on ne fait rien
        ...
        lastSaveHash = hash; // on mémorise ce que nous venons d’envoyer
    ...
    function startPollingLoop(user) {
      ...
      // Upload de secours au cas où le polling manquerait quelque chose (ex. erreur ponctuelle)
      setInterval(async () => {
        try {
    ...
    async function startSync() {
      ...
      waitForAuth(async (user) => {
        ...
        // (Optionnel) démarrer le mécanisme de sauvegarde interne de l’émulateur – ne sert plus à rien mais ne fait de mal
        waitForEmulator(() => {
          try {
        ...
        // Laisser un peu de temps à l’émulateur pour charger la ROM
        await new Promise(r => setTimeout(r, 4000));

        // Télécharger éventuellement une sauvegarde déjà présente sur le worker
        await downloadAndInjectSave(user);

        // Démarrer le polling qui détecte les changements dans IDB et upload quand nécessaire
        startPollingLoop(user);

        // Upload automatique à la fermeture de l’onglet / du navigateur
        window.addEventListener('beforeunload', () => {
          console.log('[SaveSync] beforeunload – upload forcé');
          uploadSaveIfChanged(user);
        });

        // Permettre à gba.js de déclencher un upload forcé via postMessage
    ...
    /* -------------------------- LANCEMENT -------------------------- */
    // Attendre que Firebase soit totalement initialisé
    if (window.dispatchEvent && typeof window.dispatchEvent === 'function') {
      window.addEventListener('firebase-ready', () => {
        startSync();
      });
    } else {
      // Fallback : si l'événement n'existe pas (dev), lance immédiatement
      startSync();
    }
